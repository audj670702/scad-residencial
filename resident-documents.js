...
function renderDocs(){
  $('residentDocsCount').textContent=String(residentDocs.length);const box=$('residentDocsList');
  if(!residentDocs.length){box.innerHTML='<div class="empty-state"><strong>Sin documentos disponibles</strong><span>No tienes documentos asignados en este momento.</span></div>';return}
  box.innerHTML=residentDocs.map(d=>{const meta=[d.categoria||'Sin categoría',fmtDate(d.fechaDocumento)].filter(Boolean).join(' · ');return `<article class="resident-doc-card"><div class="resident-doc-main"><span class="resident-doc-icon">📄</span><div class="resident-doc-copy"><strong>${esc(d.titulo||'Documento')}</strong><small>${esc(meta)}</small>${d.descripcion?`<p>${esc(d.descripcion)}</p>`:''}</div></div><div class="resident-doc-actions"><button type="button" data-rdoc-view="${esc(d.documentoId)}">Ver PDF</button>${d.descargable?`<button type="button" data-rdoc-download="${esc(d.documentoId)}">Descargar</button>`:''}</div></article>`}).join('');
  box.querySelectorAll('[data-rdoc-view]').forEach(b=>b.onclick=()=>openPdf(b.dataset.rdocView,false));box.querySelectorAll('[data-rdoc-download]').forEach(b=>b.onclick=()=>openPdf(b.dataset.rdocDownload,true));
}
