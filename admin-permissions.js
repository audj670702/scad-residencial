...
function ensureUI(){
 if(!$('specialPermissionsEntry')){
  ...
  if(toolbar)toolbar.insertAdjacentHTML('afterend','<div class="special-permissions-entry" id="specialPermissionsEntry"><button class="special-permissions-btn" id="openSpecialPermissions" type="button"><span>Permiso de</span><span>Control de Accesos</span></button></div>');
 }
 if(!$('specialPermissionsSheet'))document.body.insertAdjacentHTML('beforeend',`<div class="special-permissions-sheet" id="specialPermissionsSheet"><div class="special-permissions-panel"><button class="special-permissions-close" id="closeSpecialPermissions" type="button">×</button><p class="special-permissions-kicker">ADMINISTRACIÓN</p><h2>Permiso de Control de Accesos</h2><p class="special-permissions-intro">Acceso al Módulo de Control de Accesos.</p><div class="special-permissions-list" id="specialPermissionsList"></div><p class="special-permissions-status" id="specialPermissionsStatus"></p></div></div>`);
...
function renderUsers(){
 ...
 $('specialPermissionsList').innerHTML=active.map((u,i)=>{const originalIndex=users.indexOf(u),inherited=u.controlAccesosHeredado===true,noMember=!String(u.memberId||'').trim();const detail=inherited?`Incluido por rol: ${esc(u.rolResidencial||'')}`:(u.controlAccesosDirecto===true?'Acceso al módulo activo':'Sin acceso al módulo');return `<div class="special-user-row"><div class="special-user-copy"><strong>${esc(u.nombreCompleto||u.nombre||'Usuario')}</strong><span>${detail}</span></div><label class="special-permission-switch"><input type="checkbox" data-special-user="${originalIndex}" ${u.controlAccesos===true?'checked':''} ${(inherited||noMember)?'disabled':''}><span class="special-permission-slider"></span></label></div>`}).join('');
...
function requestChange(e){
 ...
 const text=requested?`Vas a autorizar a ${name} el acceso al Módulo de Control de Accesos. Podrá validar visitantes y registrar entradas y salidas.`:`Vas a retirar a ${name} el acceso al Módulo de Control de Accesos.`;
 const overlay=document.createElement('div');overlay.className='permission-confirm';overlay.innerHTML=`<div class="permission-confirm-card"><div class="permission-confirm-icon">!</div><h3>Confirmar permiso de Control de Accesos</h3><p>${esc(text)}</p><div class="permission-confirm-actions"><button class="permission-cancel" type="button">Cancelar</button><button class="permission-critical" type="button">${requested?'Autorizar acceso':'Retirar acceso'}</button></div></div>`;document.body.appendChild(overlay);
...
async function savePermission(user,requested){
 const status=$('specialPermissionsStatus');status.textContent=requested?'Autorizando acceso…':'Retirando acceso…';
 try{const d=await postStatus({usuarioId:user.id||user._id||'',permisoControlAccesos:requested});if(!d?.ok)throw Error(d?.reason||'PERMISO_UPDATE_ERROR');user.controlAccesos=requested;user.controlAccesosDirecto=requested;user.controlAccesosHeredado=false;status.textContent=requested?'Acceso al módulo autorizado correctamente.':'Acceso al módulo retirado correctamente.';renderUsers()}catch(err){console.error(err);status.textContent='No fue posible actualizar el permiso.'}
}
