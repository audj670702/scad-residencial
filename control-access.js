...
function renderHistory(){
 $('controlAccessHistoryCount').textContent=String(historyItems.length);const box=$('controlAccessHistoryList');if(!historyItems.length){box.innerHTML='<div class="cacc-empty"><strong>Sin historial</strong><span>Todavía no hay ciclos cerrados ni inconsistencias registradas.</span></div>';return}
 box.innerHTML=historyItems.map(v=>`<article class="cacc-card"><div class="cacc-card-head"><div><h3>${esc(v.nombreVisitante||'Visitante')}</h3><span class="cacc-code-pill">${esc(v.codigoAcceso||'')}</span></div><span class="cacc-status ${esc(String(v.estatus||'').toLowerCase())}">${esc(v.estatus||'')}</span></div><div class="cacc-detail-grid"><div><span>Destino</span><strong>${esc(v.destino||'—')}</strong></div><div><span>Entrada</span><strong>${esc(fmt(v.fechaEntrada))}</strong></div><div><span>Salida</span><strong>${esc(fmt(v.fechaSalida))}</strong></div><div><span>Operación</span><strong>Entrada: ${esc(v.entradaPorNombre||v.entradaPorMemberId||'—')}<br>Salida: ${esc(v.salidaPorNombre||v.salidaPorMemberId||'—')}</strong></div></div>${v.estatus==='Inconsistencia'?'<div class="cacc-warning">Entrada registrada sin salida después de 24 horas.</div>':''}${commentsHtml(v.comentarios)}</article>`).join('');
}
async function loadInside(){try{const d=await getFn(FN.inside);if(!d?.ok)throw Error(d?.reason||'DENTRO_ERROR');insideItems=Array.isArray(d.visitantes)?d.visitantes:[];historyItems=Array.isArray(d.historial)?d.historial:[];renderInside();renderHistory()}catch(e){console.error(e);insideItems=[];historyItems=[];renderInside();renderHistory()}}
