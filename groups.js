...
function showOnly(view){['authView','appView','adminView','usersView','groupsView','documentsAdminView'].forEach(id=>{const el=$(id);if(el)el.hidden=id!==view})}
...
function newGroup(){activeGroup=null;members=[];allUsers=[];$('groupId').value='';$('groupName').value='';$('groupDescription').value='';$('groupSheetTitle').textContent='Nuevo grupo';$('groupMembersBlock').hidden=true;message('');$('groupSheet').classList.add('open')}
async function openGroup(grupoId){activeGroup=groups.find(g=>g.grupoId===grupoId)||null;if(!activeGroup)return;$('groupId').value=activeGroup.grupoId;$('groupName').value=activeGroup.nombre||'';$('groupDescription').value=activeGroup.descripcion||'';$('groupSheetTitle').textContent=activeGroup.nombre;$('groupMembersBlock').hidden=false;$('groupUserSearch').value='';$('groupUserResults').innerHTML='';message('Cargando integrantes…');$('groupSheet').classList.add('open');try{const [m,u]=await Promise.all([postFn(FN.members,{grupoId}),getFn(FN.users)]);if(!m?.ok||!u?.ok)throw Error('LOAD');members=m.integrantes||[];allUsers=u.usuarios||[];renderMembers();message('')}catch(e){console.error(e);message('No fue posible cargar los integrantes.')}}
...
function renderMembers(){$('groupMemberCount').textContent=String(members.length);$('groupMembers').innerHTML=members.length?members.map(u=>person(u,'remove')).join(''):'<div class="empty-state compact"><span>Sin integrantes.</span></div>';bindMemberButtons($('groupMembers'));renderUserResults()}
function renderUserResults(){const q=$('groupUserSearch').value.trim().toLowerCase();if(q.length<2){$('groupUserResults').innerHTML='';return}const ids=new Set(members.map(x=>x.memberId));const result=allUsers.filter(u=>!ids.has(u.memberId)&&[u.nombreCompleto,u.email,u.vivienda].some(v=>String(v||'').toLowerCase().includes(q))).slice(0,12);$('groupUserResults').innerHTML=result.length?result.map(u=>person(u,'add')).join(''):'<div class="empty-state compact"><span>Sin coincidencias.</span></div>';bindMemberButtons($('groupUserResults'))}
...
async function toggleGroup(grupoId,activo){try{const d=await postFn(FN.state,{grupoId,activo});if(!d?.ok)throw Error(d?.reason||'ERROR');await refreshGroups()}catch(e){alert(String(e?.message||'No fue posible cambiar el estado del grupo.').includes('LIMITE')?'Ya existen 5 grupos activos. Desactiva uno para liberar un cupo.':'No fue posible cambiar el estado del grupo.')}}
