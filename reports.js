...
function wixImageToHttp(value){const raw=String(value||'').trim();if(!raw)return'';if(/^https?:\/\//i.test(raw))return raw;const m=raw.match(/^wix:image:\/\/v1\/([^/]+)\//i);return m?`https://static.wixstatic.com/media/${m[1]}`:''}
function readEvidence(file){return new Promise((resolve,reject)=>{if(!file){resolve(null);return}const allowed=['image/jpeg','image/png','image/webp'];if(!allowed.includes(String(file.type||'').toLowerCase()))return reject(new Error('EVIDENCIA_FORMATO_INVALIDO'));if(file.size>5*1024*1024)return reject(new Error('EVIDENCIA_TAMANO_INVALIDO'));const reader=new FileReader();reader.onload=()=>resolve({base64:String(reader.result||''),mimeType:String(file.type||'').toLowerCase(),fileName:String(file.name||'evidencia')});reader.onerror=()=>reject(new Error('EVIDENCIA_LECTURA_ERROR'));reader.readAsDataURL(file)})}
...
function render(){
 ...
 if(!items.length){box.innerHTML='<div class="rrep-empty"><strong>Sin reportes</strong><span>No tienes reportes registrados.</span></div>';return}
 box.innerHTML=items.map(r=>{
   const evidenceUrl=wixImageToHttp(r.evidencia);
   const seguimiento=String(r.seguimiento||'').trim();
   const seguimientoBlock=r.estatus==='Reportado'
     ?'<div class="rrep-followup pending"><strong>Pendiente de atención</strong><span>Administración aún no ha iniciado el seguimiento.</span></div>'
     :`<div class="rrep-followup"><strong>${r.estatus==='Cerrado'?'Conclusión de Administración':'Seguimiento de Administración'}</strong><p>${esc(seguimiento||'Sin detalle de seguimiento.')}</p><div class="rrep-followup-meta">${r.fechaAtencion?`Atención: ${esc(fmt(r.fechaAtencion))}`:''}${r.fechaCierre?` · Cierre: ${esc(fmt(r.fechaCierre))}`:''}</div></div>`;
   return `<article class="rrep-card"><div class="rrep-card-head"><div><span class="rrep-category">${esc(r.categoria||'Otros')}</span><h3>${esc(r.asunto||'Reporte')}</h3></div><span class="rrep-state ${stateClass(r.estatus)}">${esc(r.estatus||'Reportado')}</span></div><p class="rrep-description">${esc(r.descripcion||'')}</p><div class="rrep-meta"><span><strong>Folio:</strong> ${esc(r.reporteId||'—')}</span><span><strong>Fecha:</strong> ${esc(fmt(r.fechaReporte))}</span></div>${evidenceUrl?`<div class="rrep-evidence"><img src="${esc(evidenceUrl)}" alt="Evidencia del reporte"/></div>`:''}${seguimientoBlock}</article>`
 }).join('')
}
async function load(){$('residentReportsList').innerHTML='<div class="rrep-empty"><strong>Cargando reportes…</strong></div>';try{const d=await getFn(FN.list);if(!d?.ok)throw Error(d?.reason||'REPORTES_ERROR');items=Array.isArray(d.reportes)?d.reportes:[];render()}catch(e){console.error('SCaD Residencial reportes:',e);items=[];$('residentReportsCount').textContent='0';$('residentReportsList').innerHTML='<div class="rrep-empty"><strong>No fue posible cargar Reportes</strong></div>'}}
async function save(e){e.preventDefault();const file=$('residentReportEvidence').files?.[0]||null;$('residentReportSave').disabled=true;$('residentReportStatus').textContent=file?'Preparando evidencia…':'Enviando reporte…';try{const evidencia=await readEvidence(file);const payload={categoria:$('residentReportCategory').value.trim(),asunto:$('residentReportSubject').value.trim(),descripcion:$('residentReportDescription').value.trim(),...(evidencia?{evidencia}:{})};const d=await postFn(FN.save,payload);if(!d?.ok)throw Error(d?.reason||'REPORTE_GUARDAR_ERROR');$('residentReportSheet').classList.remove('open');await load()}catch(err){const reason=String(err?.message||err||'');$('residentReportStatus').textContent=reason.includes('EVIDENCIA_FORMATO_INVALIDO')?'La evidencia debe ser JPG, PNG o WEBP.':reason.includes('EVIDENCIA_TAMANO_INVALIDO')?'La evidencia debe pesar máximo 5 MB.':'No fue posible registrar el reporte.'}finally{$('residentReportSave').disabled=false}}
inject();
